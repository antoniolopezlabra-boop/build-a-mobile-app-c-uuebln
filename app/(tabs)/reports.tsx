...
export default function ReportsScreen() {
  const { canViewReports, isPremium, canUseCollaborators, loading: planLoading } = usePlan();
  const { user } = useAuth();
  ...
  // Sub-tab para Premium: 'general' o 'equipo'
  const [reportTab, setReportTab]   = useState<'general' | 'equipo'>('general');
  // Rango de equipo: mes actual por defecto
  ...
  const loadData = async (forceRefresh = false, isPullRefresh = false, silent = false) => {
    ...
    try {
      ...
      // Cargar stats de equipo si es Premium
  ...
  const loadStaffStats = async (userId: string, range: 'semana' | 'mes' | 'todo') => {
    try {
      // Calcular fecha inicio seg\xFAn rango
      ...
      // Cargar citas con staff_id en el rango
      let query = supabase
        .from('appointments')
      ...
      // Calcular m\xE9tricas por staff
      const result: StaffStat[] = staffList.map(m => {
        ...
        const noshow    = mine.filter((a: any) => a.status === 'No asisti\xF3').length;
  ...
  // M\xE1ximo de citas entre todos los staff (para la barra proporcional)
  ...
  return (
    <SafeAreaView style={[s.container, { backgroundColor: tc.bg }]}>
      ...
      >
        ...
        {/* Tab principal: General / Mi equipo (solo Premium con staff) */}
                  ...
                  <View key={st.id} style={[s.staffCard, { backgroundColor: tc.surface }]}>
                    ...
                    <View style={s.staffCardHeader}>
                      ...
                      <View style={[s.rankBadge, { backgroundColor: idx === 0 ? '#FEF3C7' : idx === 1 ? '#F1F5F9' : '#FFF7ED' }]}>
                        <Text style={[s.rankNum, { color: idx === 0 ? '#92400E' : idx === 1 ? '#475569' : '#9A3412' }]}>
                          #{idx + 1}
                        </Text>
                      </View>
                      {/* Avatar */}
                    ...
                    {/* Barra proporcional de actividad */}
                    <View style={[s.staffBarBg, { backgroundColor: tc.inputBg }]}>
                      <View
                        style={[s.staffBarFill, {
                          backgroundColor: st.color,
                          width: `${Math.round((st.total / maxStaffTotal) * 100)}%` as any,
                        }]}
                      />
                    </View>

                    {/* Chips de m\xE9tricas */}
