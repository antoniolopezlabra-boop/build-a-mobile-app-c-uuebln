...
export default function ScheduleSettingsScreen() {
  ...
  const loadSchedule = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        // Merge loaded data with all days (in case some days are missing)
  ...
  const updateTime = (dayOfWeek: number, field: 'startTime' | 'endTime', time: string) => {
    setSchedule((prev) =>
      prev.map((day) => (day.dayOfWeek === dayOfWeek ? { ...day, [field]: time } : day))
    );
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Save each day individually using PUT /api/business-hours/{dayOfWeek}
      await Promise.all(
        schedule.map((day) =>
          apiPut(`/api/business-hours/${day.dayOfWeek}`, {
      ...
      console.log('[Schedule] Schedule saved');
    } catch (error: any) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ConfirmModal
        ...
        title="¡Guardado!"
        message="El horario de atención se actualizó correctamente."
        buttons={[
          {
            text: 'Aceptar',
            onPress: () => {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Text style={styles.description}>
          Configura los días y horarios en los que tu negocio está abierto. Esto se usará para
          validar las citas.
        </Text>

        {DAYS.map((day) => {
          const daySchedule = getDaySchedule(day.value);
          const startTimeText = formatTime(daySchedule.startTime);
          const endTimeText = formatTime(daySchedule.endTime);
