  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('promo_codes').insert({
        ...
        discount_type: isFree ? 'full' : 'percent',
        discount_value: isFree ? 100 : parseInt(discountValue),
        duration_days: isPermanent ? null : parseInt(durationDays),
      ...
      console.error('[PromoCodes] Save error:', err);
    } finally {
      setSaving(false);
    }
  };
      ...
      {showForm && (
        <ScrollView style={styles.formScroll} keyboardShouldPersistTaps="handled">
          <View style={styles.form}>
            ...
            <Text style={styles.fieldLabel}>Para quién es / motivo</Text>
            <TextInput
              style={styles.input}
            ...
            <View style={styles.switchRow}>
              <Text style={styles.fieldLabel}>Duración permanente</Text>
            ...
            <TouchableOpacity style={styles.saveBtn} onPress={handleSave} disabled={saving}>
              <Text style={styles.saveBtnText}>{saving ? 'Guardando...' : '✅ Crear código'}</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      )}
              ...
              codes.map((c) => (
                <View key={c.id} style={[styles.card, !c.isActive && styles.cardInactive]}>
                  ...
                  <View style={styles.cardBottom}>
                    ...
                    <Text style={styles.dateText}>Creado: {new Date(c.createdAt).toLocaleDateString('es-MX')}</Text>
                    <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  formTitle: { fontSize: 16, fontWeight: '700', color: '#F8FAFC', marginBottom: 4 },
